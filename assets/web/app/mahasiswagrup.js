...
function loadprodi(vselector) {
    $(vselector).empty();
    appAjax("api/loadprodi", {}, false).done(function (vRet) {
...
$('#iduser').select2({
    ...
    ajax: {
        dataType: 'json',
        url: vBase_url+'app/mahasiswagrup/cariakun',
        delay: 800,
        type: "post",
...
function loadTabel() {
    dtTable = $('.table').DataTable({
        ...
        "ajax": {
            "url": vBase_url + "app/mahasiswagrup/read",
            "dataType": "json",
            "type": "POST",
            "data": function (d) {
...
$(document).on("click", ".editRow", function (e) {
    ...
    appAjax("app/mahasiswagrup/cari", cari).done(function (vRet) {
        if (vRet.status) {
            var myModal = new bootstrap.Modal(document.getElementById('modal-form'), {
                backdrop: 'static',
                keyboard: false,
...
$(document).on("click", ".deleteRow", function (e) {
    ...
    fDelete(submitVal, "app/mahasiswagrup/delete");
});
...
function loadoutput(db) {
    ...
    $('#idprodi').val(db['idprodi']).trigger("change");
...
$("#form").submit(function (e) {
    ...
    if ($(this).validationEngine('validate')) {
        appAjax("app/mahasiswagrup/simpan", formVal).done(function (vRet) {
            showNotification(vRet.status, vRet.pesan);
            if($("id").val()=="")
                resetform();
            if (vRet.status) {
