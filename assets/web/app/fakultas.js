...
function loadTabel() {
    dtTable = $('.table').DataTable({
        ...
        "ajax": {
            "url": vBase_url + "app/fakultas/read",
            "dataType": "json",
            "type": "POST",
...
$("#formweb").submit(function(e) {
    ...
    let formVal = {
        id: $("#formweb #id").val(),
        fakultas: $("#formweb #fakultas").val(),
    };
    if ($('#formweb').validationEngine('validate')) {
        appAjax("app/fakultas/simpan", formVal).done(function(vRet) {
...
$(document).on("click",".editRow", function(e) {
    ...
    appAjax("api/carigeneral", formVal).done(function(dataLoad) {
        if (dataLoad.status) {
            ...
            $("#formweb #fakultas").val(dtweb.fakultas);
