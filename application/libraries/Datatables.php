...
class Datatables
{
	...
	public function order_by($fld, $method = "ASC")
	{
		$this->order_by[]	= array(
			'fld'		=> $fld,
			'method'	=> $method
		);
		$this->ci->db->order_by($fld, $method);
		return $this;
	}
	...
	private function get_total_results($filtering = false)
	{
		...
		foreach ($this->order_by as $val) {
			$this->ci->db->order_by($val['fld'], $val['method']);
		}
		...
		$cek = ($this->distinct) ? $this->distinct : "";
		if (strlen($cek) > 0) {

			$this->ci->db->distinct($this->distinct);
			$this->ci->db->select($this->oselect);
		} else {
			$cek = ($this->count_column) ? $this->count_column : "";
			if (strlen($cek) > 0) {
