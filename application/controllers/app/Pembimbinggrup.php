...
class Pembimbinggrup extends CI_Controller
{
    private $d = array(
        "tbName" => "hakakses",
        "idgrup" => "5",
        ...
        "primaryKey" => "id",
        "web" => array(
            "title" => "Akun Pembimbing KKN",
            "parent" => array("pengaturan"),
            "modul" => "app/pembimbinggrup",
            "view"  => "app",
            "page"    => "Admin",
        ),
    );
    ...
    public function index()
    {
        ...
        $this->d['web']['loadview'] = $this->d['web']['view'] . "/pembimbinggrup";
        ...
        $this->d['web']['importJs'] = array(
            base_url('assets/web/app/pembimbinggrup.js?' . date("ymdhis")),
        );
    ...
    public function read()
    {
        ...
        $this->datatables->join("pembimbing as a", "h.id=a.idhakakses", "left");
        ...
        foreach ($retVal['data'] as $index => $dp) {
            ...
            if (!$dp['id']) {
                ...
                //$pembimbingadd = "<a class='dropdown-item setPembimbing' data-pilih='" . $no . "' data-id='" . $dp['idhakakses'] . "' href='#'><i class='bi bi-node-plus'></i> Set Pembimbing</a>";
            }
            $tmp['status'] = $status;
        ...
        foreach ($id as $i => $dp) {
            ...
            if ($dp != "") {
                ...
                if ($run->status) {
                    ...
                    if ($hakakses) {
                        $status = $this->Model_data->delete_pembimbing_data($hakakses->iduser);
                        if ($status === true) {
                            $retVal['pesan'][$i] = array("Hapus data pembimbing berhasil dilakukan");
                            $retVal['status'] = true;
                        } else {
    ...
    // public function setPembimbing($iduser=null,$dataHakakses=null)
    // {
    //     $dataGrup = array(
    //         "idgrup" => $retVal['id'],
    //         "iduser" => $dataSave['idgrup'],
    //         "statuspeg" => $dataSave['statuspeg'],
    //         "nip" => $dataSave['nip'],
    //     );
    //     $retVal = $this->Model_data->save($dataGrup, "pembimbing", null, true);
    // }
    ...
    public function simpan()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if (!$dataSave['idpembimbing']) {
                ...
                $retVal = $this->Model_data->update($kondgrup, $dataGrup, "pembimbing", null, true);
            }
            //     $this->setPembimbing($dataSave['idhakakses']);
            //debug($dataGrup);
        } else {
