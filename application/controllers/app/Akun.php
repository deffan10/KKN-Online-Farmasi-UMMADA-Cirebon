...
class Akun extends CI_Controller
{
    ...
    public function delete()
    {
		...
		$retVal = ["status" => false, "pesan" => [], "login" => true];
        $id = $this->input->post('idTerpilih');
		$iduser_sess = $this->session->userdata("iduser");
        $anySuccess = false;
        ...
        foreach ($id as $i => $dp) {
			...
			if ($dp == $iduser_sess) {
                $retVal['pesan'][$i] = "Tidak dapat menghapus akun sendiri.";
                continue;
            }
			
			$run = akses_akun("delete", $this->otentikasi, $this->d['tbName'], $dp);
            if ($run->status) {
                $this->db->where('iduser', $dp)->delete('hakakses');
                $kond = array(
                    array("where", "id", $dp),
                );
                ...
                #$retVal['status'] = true;
				if ($runquery['status']) {
                    $anySuccess = true; // Status sukses hanya jika query delete berhasil
                }
            } else {
        ...
        $retVal['status'] = $anySuccess; // Status akhir ditentukan oleh keberhasilan query
        die(json_encode($retVal));
    }
